...
@Component({
  selector: 'app-day11-start',
  standalone: true,
  imports: [RouterLink, CodeBlockComponent, CollapsibleComponent],
  template: `
    <div class="lesson-content">
      <div class="page-header">
        <span class="act-label">Day 11 · Starting Point</span>
        <h1>🎬 Get BingeBoard Running</h1>
        <p class="subtitle">The reactive-forms BingeBoard from Day 10 — a working ReviewForm posting into a signal-based ReviewsService, rendered with a spoiler reveal, two quick-fill buttons, and a live character count, ready for validation.</p>
      </div>

      <div class="info-box">
        <strong>Two ways to get there:</strong> clone and run the Day 11 starter, or compare the reference files below with your end-of-Day-10 project.
      </div>
      ...
      <section class="lesson-framework">
        ...
        <ul>
          ...
          <li>Run <code>cd starters/bingeboard-day11 && npm install && npm start</code>.</li>
          <li>Open <code>http://localhost:4200</code>.</li>
        </ul>
        <app-code-block lang="bash" [code]="cloneCommand" />
        <p><a href="https://github.com/megabottty/mtech-angular-lab-data-binding/tree/main/starters/bingeboard-day11" target="_blank" rel="noopener">Browse starter files on GitHub</a>.</p>
      </section>

      <section class="lesson-framework">
        <h3>Option B — Bring your own project</h3>
        <p>Day 10 should leave you with a working <code>ReviewForm</code>: a <code>Review</code> model, a signal-based <code>ReviewsService</code>, a <code>FormBuilder</code>-built form with no validation attached yet, reviews rendered on the show detail page with a spoiler reveal, two quick-fill buttons, and a live headline character count. Every file below is the exact end-of-Day-10 state — copy it in before following the acts.</p>
        ...
        <app-collapsible icon="📄" label="src/app/models/review.ts"><app-code-block lang="typescript" file="src/app/models/review.ts" [code]="reviewModelCode" /></app-collapsible>
        ...
        <app-collapsible icon="📄" label="src/app/services/reviews.ts"><app-code-block lang="typescript" file="src/app/services/reviews.ts" [code]="reviewsServiceCode" /></app-collapsible>

        <h4>Review form</h4>
        <app-collapsible icon="📄" label="src/app/review-form/review-form.ts"><app-code-block lang="typescript" file="src/app/review-form/review-form.ts" [code]="reviewFormTsCode" /></app-collapsible>
        <app-collapsible icon="📄" label="src/app/review-form/review-form.html"><app-code-block lang="html" file="src/app/review-form/review-form.html" [code]="reviewFormHtmlCode" /></app-collapsible>
      ...
      <section class="lesson-framework">
        ...
        <ul>
          ...
          <li><strong>Day 10:</strong> a <code>Review</code> model, a signal-based <code>ReviewsService</code>, a <code>FormBuilder</code>-built <code>ReviewForm</code> with no validation attached yet, reviews rendered with a spoiler reveal and empty state, two quick-fill buttons, and a live headline character count.</li>
        </ul>
      </section>
      ...
      <section class="lesson-framework">
        ...
        <ul class="task-checklist">
          ...
          <li><span class="checkbox">✅</span> A show detail page renders <code>ReviewForm</code> above a "Reviews" section.</li>
          <li><span class="checkbox">✅</span> Submitting a review appears immediately in that show's own review list.</li>
          <li><span class="checkbox">✅</span> A spoiler review's body stays hidden until you click "Reveal review."</li>
          <li><span class="checkbox">✅</span> Both quick-fill buttons work, and the headline character count updates live.</li>
          <li><span class="checkbox">✅</span> Nothing currently stops you from submitting a completely empty review.</li>
        </ul>
      </section>

      <div class="warning-box">If a check fails, return to Day 10 before continuing with today's acts.</div>

      <div class="nav-footer">
        <a routerLink="/day10/lab" class="btn-secondary">← Day 10 Lab</a>
        <a routerLink="/day11/act1" class="btn-primary">Act 1: Validators &amp; Control State →</a>
      </div>
    </div>
  `
})
export class Day11StartComponent {
  cloneCommand = `git clone https://github.com/megabottty/mtech-angular-lab-data-binding.git
cd mtech-angular-lab-data-binding/starters/bingeboard-day11
npm install
npm start`;
...
export class ReviewsService {
  ...
  reviewFormTsCode = `import { Component, inject, input } from '@angular/core';
...
export class ReviewForm {
  ...
  reviewFormHtmlCode = `<form [formGroup]="reviewForm" (ngSubmit)="submit()">
  <label>
    Rating (0-10)
  ...
  <label>
    Headline
    <input type="text" formControlName="headline" maxlength="60" placeholder="Sum it up in one line" />
  </label>
  <p class="char-count" [class.over-limit]="reviewForm.controls.headline.value.length > 60">
...
.card-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 16px; }`;
  showDetailPageCode = `import { Component, computed, inject, input, signal } from '@angular/core';
import { RouterLink } from '@angular/router';
import { ShowsService } from '../../services/shows';
import { WatchlistService } from '../../services/watchlist';
import { ReviewsService } from '../../services/reviews';
