...
@Component({
  selector: 'app-day7-start',
  standalone: true,
  imports: [RouterLink, CodeBlockComponent, CollapsibleComponent],
  template: `
    <div class="lesson-content">
      <div class="page-header">
        <span class="act-label">Day 7 · Starting Point</span>
        <h1>🎬 Get BingeBoard Running</h1>
        <p class="subtitle">The exact end-of-Day-6 BingeBoard — filterable cards wrapped in reusable, collapsible panels — ready to run, so you spend today's time on services instead of retyping six days of prior code.</p>
      </div>
      ...
      <section class="lesson-framework">
        ...
        <ul>
          <li><strong>Clone the teaching site's repo</strong>, which includes this starter under <code>starters/bingeboard-day7/</code>.</li>
          <li><strong>Install and run it:</strong> <code>cd starters/bingeboard-day7 && npm install && npm start</code>.</li>
        ...
        <p style="margin-top: 8px;">
          ...
          <a href="https://github.com/megabottty/mtech-angular-lab-data-binding/tree/main/starters/bingeboard-day7" target="_blank" rel="noopener">starters/bingeboard-day7</a>.
        </p>
      </section>
      ...
      <section class="lesson-framework">
        ...
        <p>
          Already have Day 6's BingeBoard building in your own project? You can skip Option A entirely —
        ...
        <app-collapsible icon="📄" label="src/styles.css">
          <app-code-block lang="css" file="src/styles.css" [code]="globalCssCode" />
        </app-collapsible>
        ...
        <h4 style="margin-top: 20px;">Panel (new on Day 6)</h4>
        <app-collapsible icon="📄" label="src/app/panel/panel.ts">
          <app-code-block lang="typescript" file="src/app/panel/panel.ts" [code]="panelTsCode" />
        </app-collapsible>
        <app-collapsible icon="📄" label="src/app/panel/panel.html">
          <app-code-block lang="html" file="src/app/panel/panel.html" [code]="panelHtmlCode" />
        </app-collapsible>
        <app-collapsible icon="📄" label="src/app/panel/panel.css">
          <app-code-block lang="css" file="src/app/panel/panel.css" [code]="panelCssCode" />
        </app-collapsible>
      ...
      <section class="lesson-framework">
        ...
        <ul>
          ...
          <li><strong>Day 6:</strong> a reusable <code>Panel</code> built on <code>&lt;ng-content&gt;</code> with a <code>[panel-actions]</code> header slot and its own collapse toggle; <code>WatchlistPanel</code> projects a custom empty state; global CSS custom properties, a real card grid, and a sticky header bar.</li>
        </ul>
      </section>
      ...
      <section class="lesson-framework">
        ...
        <ul class="task-checklist">
          <li><span class="checkbox">✅</span> Eight show cards render in a responsive grid that reflows as you resize the window.</li>
          ...
          <li><span class="checkbox">✅</span> <strong>Clear filters</strong> sits in the Browse panel's header bar (not in the body) and resets the filters.</li>
          <li><span class="checkbox">✅</span> "Add to watchlist" puts the show in the watchlist panel and immediately greys out that card's button.</li>
          <li><span class="checkbox">✅</span> With an empty watchlist you see "Nothing yet - go browse all 8 shows!" — that text is written in <code>app.html</code> and projected into <code>WatchlistPanel</code>.</li>
          <li><span class="checkbox">✅</span> The ▾ button in either panel header collapses and expands that panel, and the chevron rotates.</li>
        </ul>
      </section>

      <div class="warning-box">
        If any of those checks fail, don't push forward into Day 7 yet — go back to Day 6's Acts/Lab and get it working first. Everything today builds on top of this exact state.
      </div>

      <div class="info-box">
        <strong>One deliberate omission.</strong> Day 6's Act 2 had you add <code>console.log</code> calls to <code>ShowCard</code>'s lifecycle hooks, and Lab task 3 added a "seconds on screen" ticker. Those were lifecycle <em>proofs</em>, not features — eight cards logging every second gets noisy fast — so they aren't in this starter. If you still have them, deleting them now is the right call.
      </div>

      <div class="nav-footer">
        <a routerLink="/day6/lab" class="btn-secondary">← Day 6 Lab</a>
        <a routerLink="/day7/act1" class="btn-primary">Act 1: State That Outlives Components →</a>
      </div>
    </div>
  `
})
export class Day7StartComponent {
  cloneCommand = `git clone https://github.com/megabottty/mtech-angular-lab-data-binding.git
cd mtech-angular-lab-data-binding/starters/bingeboard-day7
npm install
npm start`;
...
.tracker {
  ...
  globalCssCode = `/* BingeBoard - global styles.
