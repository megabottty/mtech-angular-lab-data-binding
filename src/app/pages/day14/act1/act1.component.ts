...
@Component({
  ...
  template: `
    <div class="lesson-content">
      ...
      <div class="info-box">
        <strong>Warm-up drill:</strong> start from yesterday's Browse demo, open DevTools, switch the Network tab to <code>Offline</code>, and intentionally break a search. The eternal <code>Searching…</code> message is the bug this act is about.
      </div>
      ...
      <section class="lesson-framework">
        ...
        <ul>
          ...
          <li><strong>Why It Matters:</strong> Production apps are not judged by how they behave when everything works. They are judged by whether they stay honest and usable when the network, the server, or the request goes sideways.</li>
      ...
      <app-lesson-step stepId="d14-act1-warmup-problem" [stepNumber]="1" title="The Warm-Up — Breaking It On Purpose">
        ...
        <app-collapsible icon="🧩" label="Deep Dive — Why an eternal spinner is a real bug">
          ...
          <p style="margin-top: 12px;">That is why error handling is part of correctness, not just polish. Honest state means the screen's story matches the actual state of the network request.</p>
        </app-collapsible>
        <div class="outcome-check">✅ <strong>Expected outcome for this step:</strong> You can articulate the three questions every data-fetching UI must answer and explain why an eternal spinner is a bug, not a neutral default.</div>
      </app-lesson-step>
      ...
      <app-lesson-step stepId="d14-act1-error-signal" [stepNumber]="2" title="Adding an error Signal + Retry to Browse">
        ...
        <div class="info-box">
          <strong>Style note that matters:</strong> prefer <code>.subscribe(&#123; next: ..., error: ... &#125;)</code> over the older positional form <code>.subscribe(nextFn, errorFn)</code>. Later today, Act 3 explicitly treats the positional-callback version as a bug to spot and fix.
        </div>
        ...
        <app-collapsible icon="✅" label="Show Answer — The behavioral proof to demo live">
          <p>Take the browser offline and search. The loading message should disappear and the friendly error box should appear. Then go back online, click <code>Retry</code>, and confirm that the same term runs again without the student retyping it.</p>
        </app-collapsible>
        <div class="outcome-check">✅ <strong>Expected outcome for this step:</strong> You can add an error signal, use the observer-object subscribe form, and build a working retry button that remembers the last search term.</div>
      </app-lesson-step>
      ...
      <app-lesson-step stepId="d14-act1-taxonomy" [stepNumber]="3" title="Error Taxonomy — Not All Failures Are Equal">
        ...
        <app-collapsible icon="💡" label="Hint — A tiny preview of retry(1)">
          <p>RxJS has a one-line operator named <code>retry(1)</code> that automatically re-attempts a flaky request once before your error handler runs. Plant the idea today, but do not over-explain it yet — Day 15 is where pipe operators get the full treatment.</p>
        </app-collapsible>
        <div class="outcome-check">✅ <strong>Expected outcome for this step:</strong> You can distinguish <code>status: 0</code>, <code>4xx</code>, and <code>5xx</code> failures and explain why each deserves a different user-facing message.</div>
      </app-lesson-step>
