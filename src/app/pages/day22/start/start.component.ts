...
@Component({
  selector: 'app-day22-start',
  standalone: true,
  imports: [RouterLink, CodeBlockComponent, CollapsibleComponent],
  template: `
    <div class="lesson-content">
      <div class="page-header">
        <span class="act-label">Day 22 · Starting Point</span>
        <h1>🎬 Get BingeBoard Running</h1>
        <p class="subtitle">The end-of-Day-21 BingeBoard: real Google sign-in, per-user watchlists, authored reviews, and locked-down Firestore rules — the exact app Day 22 puts on a public URL.</p>
      </div>
      ...
      <section class="lesson-framework">
        ...
        <ul><li>Clone the teaching repository, then enter <code>starters/bingeboard-day22/</code>.</li><li>Run <code>npm install</code>, paste your own Firebase config into <code>src/environments/environment.ts</code>, enable the Google provider in the Firebase console, then run <code>npm start</code>.</li></ul>
        <app-code-block lang="bash" [code]="cloneCommand" />
        <p>Browse it on GitHub: <a href="https://github.com/megabottty/mtech-angular-lab-data-binding/tree/main/starters/bingeboard-day22" target="_blank" rel="noopener">starters/bingeboard-day22</a>.</p>
      </section>
      <section class="lesson-framework">
        <h3>Option B — Bring your own project</h3>
        <p>Expand every source file to copy or compare the exact end-of-Day-21 reference implementation.</p>
      ...
      <section class="lesson-framework"><h3>What this code already does</h3><ul><li><strong>Days 9-17:</strong> routing, guards, real HTTP, typeahead, pipes, and a 404 page.</li><li><strong>Days 18-20:</strong> Firestore reads, watchlist CRUD, authored reviews via a server-side query.</li><li><strong>Day 21:</strong> Google sign-in, per-user watchlists, route guards with return-URL preservation, and Firestore rules that enforce ownership.</li></ul></section>
      <section class="lesson-framework"><h3>Verify before you start</h3><ul class="task-checklist"><li><span class="checkbox">✅</span> Your Firebase config is present in <code>environment.ts</code> and the Google provider is enabled.</li><li><span class="checkbox">✅</span> You can sign in, add a show, sign out, and see the watchlist empty.</li><li><span class="checkbox">✅</span> A review you post shows your name and avatar, with a Delete button only for you.</li></ul></section>
      <div class="warning-box">If a check fails, return to the day and act that built it before continuing. Day 22 does not add app features — it packages and ships the app exactly as it stands now.</div>
      <div class="nav-footer"><a routerLink="/day21/lab" class="btn-secondary">← Day 21 Lab</a><a routerLink="/day22/act1" class="btn-primary">Act 1: The Production Build →</a></div>
    </div>
  `
})
export class Day22StartComponent {
  cloneCommand = `git clone https://github.com/megabottty/mtech-angular-lab-data-binding.git
cd mtech-angular-lab-data-binding/starters/bingeboard-day22
npm install
